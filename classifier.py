...
class LlamaEmbeddingClassifier(torch.nn.Module):
	...
	def forward(self, input_ids):
		...
		'''
		# todo
		_, llama_hidden = self.llama(input_ids)
		last_llama_hidden = llama_hidden[:, -1]
		last_hidden_dropout = self.dropout(last_llama_hidden)
		logits = self.classifier_head(last_hidden_dropout)
		log_probabilities = F.log_softmax(logits, dim=-1)
